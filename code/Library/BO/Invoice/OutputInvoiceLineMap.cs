...
{
	[Serializable()]
	public class InvoiceLineMap : ClassMapping<OutputInvoiceLineRecord>
	{
		public InvoiceLineMap()
		{
			...
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`IVInvoiceLine_OID_seq`" })); map.Column("`OID`"); });
