...
{
	[Serializable()]
	public class TicketLineMap : ClassMapping<TicketLineRecord>
	{
		public TicketLineMap()
		{
			...
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`IVTicketLine_OID_seq`" })); map.Column("`OID`"); });
			Property(x => x.OidTicket, map => { map.Column("`OID_TICKET`"); map.NotNullable(false); map.Length(32768); });
