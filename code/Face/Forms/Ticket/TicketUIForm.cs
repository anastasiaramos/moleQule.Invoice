...
{
    public partial class TicketUIForm : TicketForm, IBackGroundLauncher
    {
        ...
        public TicketUIForm(Form parent)
            : this(-1, parent) {}

        public TicketUIForm(long oid, Form parent)
            : base(oid, true, parent)
        {
            InitializeComponent();
        }

        public TicketUIForm(Ticket Ticket)
            : base(null)
        {
            InitializeComponent();
		...
		private void Fecha_DTP_ValueChanged(object sender, EventArgs e)
		{
			...
			Prevision_TB.Text = _entity.PrevisionPago.ToShortDateString();
		}
        ...
        private void MedioPago_CB_SelectedIndexChanged(object sender, EventArgs e)
        {
			...
			SetMedioPago((EMedioPago)(long)MedioPago_CB.SelectedValue);
        }

        private void FormaPago_CB_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (FormaPago_CB.SelectedValue == null) return;
			_entity.PrevisionPago = Library.Common.EnumFunctions.GetPrevisionPago(_entity.EFormaPago, _entity.Fecha, _entity.DiasPago);
            Prevision_TB.Text = _entity.PrevisionPago.ToShortDateString();
        }
